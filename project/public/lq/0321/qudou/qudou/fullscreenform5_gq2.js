...
//		this._showCtrl( this.ctrlContinue );
		this.ctrlContinue = this.formEl.querySelectorAll( '.continue' );
		this.ctrlNext = this.formEl.querySelectorAll( '.next' );
...
/*		this.ctrlContinue.addEventListener( 'click', function() {
		...
		this.ctrlContinue.forEach( function( dot, pos ) {
			dot.addEventListener( 'click', function() {
				self._nextField();
			} );*/
			for(var i=0;i<this.ctrlContinue.length;i++){
				this.ctrlContinue[i].addEventListener( 'click', function() {
					self._nextField(); 
				} );
			}
			[].slice.call(this.ctrlNext).forEach( function( dot, pos ) {
				dot.addEventListener( 'click', function() {
					self._nextField();
				} )
			});
...
if( this.current==(this.fieldsCount - 1) ) {
	$('.fs-fields li:last-child').css('height','auto');
		self=this;
					...
					for(var i=1;i<7;i++){
						if(i!=2&&i!=5&&i!=6){
							...
							q[i] = q[i].substring(0,q[i].length-1);	
						}
					}
					var data = {
							sex:sex,
					...
					/*var url = 'http://kh.wqc.so/Activityother/Survey_yq9';
					$.ajax({
						type:'post',
						dataType:'json',
						url:url,
						...
						success:function(data){
							if(data.status=='成功'){
								...
								$('#bianhao').html((Array(6).join('0') + data.bianhao).slice(-6));
...
/*		if(this.current==5) {
			var sub = document.querySelector('.fs-continue');
			sub.innerHTML='提交';
		}*/
		if( !this.isLastStep ) {
			// update nav
...
/*					self._hideCtrl( self.ctrlNav );
					...
					var url = 'Activityother/Survey_orangent';
					$.ajax({
						type:'post',
						dataType:'json',
						url:url,
						...
						success:function(data){
							if(data.status=='成功'){
								...
								$('#bianhao').html((Array(5).join('0') + data.bianhao).slice(-5));
								var nextField = self.fields[ self.current ];
								classie.add( nextField, 'fs-current' );
								classie.add( nextField, 'fs-show' );
	...
	FForm.prototype._validade = function() {
		...
		if(this.current==0) {
			[].slice.call(fld.querySelectorAll('[required]')).forEach(function(el){
				if(el.value==''){
					error = 'NOVAL';
					return false;
				}
